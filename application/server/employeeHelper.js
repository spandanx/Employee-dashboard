...
exports.getFilteredEmployeesPaginated = async(month, day, pageNumber, pageSize) => {
    try {
	console.log('Called getAllEmployees() helper');
        const wallet = await Wallets.newFileSystemWallet(walletPath);
        console.log(`Wallet path: ${walletPath}`);
        ...
        // Get the contract from the network.
        const contract = network.getContract(employeeCC);
	console.log('Calling getEmployeeByMonthDayPagination(), params');
	console.log(month);
	console.log(day);
	console.log(pageNumber);
	console.log(pageSize);
//	console.log(bookmark);
        //const result = await contract.evaluateTransaction('getEmployeeByMonthDayPagination', month, day, pageSize, bookmark);
	const result = await contract.evaluateTransaction('getEmployeeByMonthDay', month, day, pageNumber, pageSize);
        //const result = await contract.evaluateTransaction('getEmployeeByMonthDay', '06', '02', 0, 1);//month, day, pageNumber, pageSize
        //onst result = await contract.evaluateTransaction('executeQuery', '{"selector":{"month":{"$eq":"06"},"day":{"$eq":"02"}},"limit":1,"skip":0}');//month, day, pageNumber, pageSize
        console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
